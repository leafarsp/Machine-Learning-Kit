...
class MLPClassifier:
    ...
    def backward_propagation(self, x, d, alpha, eta):
        ...
        for j in range(0, self.m[self.L]):
            self.l[self.L-1].eav[j] += (1/self.batch_size)*(output_d[j] - self.l[self.L-1].y[j])



        if self.__batch_cnt >= self.batch_size:

            # print(f'batch_cnt: {self.__batch_cnt} error 1st weight: {self.l[self.L - 1].eav[0]} d: {output_d[0]}, y: {self.l[self.L-1].y[0]}')
            for l in range(self.L - 1, -1, -1):
                for j in range(0, self.m[l + 1]):
                    # print(f'l={l}, j= {j}')
                    if l == (self.L - 1):
                        # equation 4.45 from Neural Networks - Simon Haykin
                        self.l[l].e[j] = self.l[l].eav[j] #(output_d[j] - self.l[l].y[j])
                    else:
            ...
            self.__batch_cnt = 0
            self.l[self.L - 1].eav = np.zeros(self.m[self.L])
